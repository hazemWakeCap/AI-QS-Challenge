...
// The IFC-element → BOQ-item register lives beside the workbook, so it is found by the same walk.
// Generated by tools/ifc_boq_map/generate_map.py; absent is a valid state (feature simply off).
var elementMapPath = builder.Configuration["Data:IfcElementMapPath"]
    ?? Path.Combine(Path.GetDirectoryName(workbookPath)!, "ifc_boq_map.csv");
builder.Services.AddSingleton<IIfcElementMapSource>(
    new IfcElementMapCsvLoader(elementMapPath, owningProjectId));
...
builder.Services.AddSingleton<IProjectSnapshotRegistry>(sp =>
    new ProjectSnapshotRegistry(sp.GetRequiredService<IProjectPanelSource>(),
        ...
        sp.GetRequiredService<IIfcElementMapSource>()));

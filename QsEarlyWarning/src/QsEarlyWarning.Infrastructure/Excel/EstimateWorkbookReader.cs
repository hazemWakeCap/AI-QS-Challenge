...
/// <summary>
/// The estimate-graph parser, extracted verbatim from <see cref="EstimateWorkbookLoader"/> so both the
/// idea-3 loader and the workbook importer share one implementation. Reads sheets 1_BOQ,
/// 2_ESTIMATE_NORMS, 3_BOQ_MAPPING, 4_ESTIMATE_DATASHEET via ClosedXML using the
...
/// contain embedded '\n', e.g. "Qty/\nUnit Work"). No arithmetic, no divisor math — raw columns only.
/// </summary>
public sealed class EstimateWorkbookReader : IEstimateWorkbookReader
{
    public EstimateModel Read(string workbookPath)
    {

...
function ConeChart({ f, currency }: { f: CentreForecast; currency: string }) {
  ...
  const gridlines = [0.25, 0.5, 0.75, 1].map((frac) => yMax * frac);
  // compact axis label so 6–7 digit amounts don't clip the left gutter
  const axis = (v: number) => v >= 1e6 ? `${(v / 1e6).toFixed(1)}M` : v >= 1e3 ? `${Math.round(v / 1e3)}k` : `${Math.round(v)}`;
  ...
  return (
    <svg width={w} height={h} className="cone">
      ...
      <text x={padL} y={y(f.bac) - 4} className="cone-lbl">BAC {money(f.bac, currency)}</text>
...
export function ForecastCone({ rev, currency = "AED" }: { rev: number; currency?: string }) {
  ...
  if (!list) return <Spinner label="Loading forecast…" />;
    ...
    <div>
      ...
      {roll && (
        <div className="muted small" style={{ marginBottom: 10 }}>
          ...
          <b> {money(roll.p10, currency)}</b> · <b>{money(roll.p50, currency)}</b> · <b>{money(roll.p90, currency)}</b> (P10·P50·P90) —
      ...
      {f && (
        <>
          <div className="panel-head">
            ...
            <span className="muted small">origin P{f.originPeriod} · {pct(f.progressPct, 0)} complete · BAC {money(f.bac, currency)} · AC {money(f.acAtOrigin, currency)}</span>
          </div>
          <ConeChart f={f} currency={currency} />
            ...
            <tbody>
              {f.increments.map((b) => (
                <tr key={b.horizon}>
                  ...
                  <td className="num">{b.available && b.p10 != null ? money(b.p10, currency) : "—"}</td>
                  <td className="num">{money(b.p50, currency)}</td>
                  <td className="num">{b.available && b.p90 != null ? money(b.p90, currency) : "—"}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="muted small" style={{ marginTop: 8 }}>
            ...
            {showFinal && f.directionalFinalCost != null && <span> — ≈ {money(f.directionalFinalCost, currency)} (BAC/CPI-style extrapolation; no final-cost ground truth on this project)</span>}
          </div>
        </>
      )}
    </div>
